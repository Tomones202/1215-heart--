...
function App() {
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      // Parse JSON error message if present to show something readable
      if (typeof errMsg === 'string' && errMsg.includes('{')) {
          try {
              // Extract JSON part if mixed
              const jsonStart = errMsg.indexOf('{');
              // Find the matching closing brace (simple logic) or last brace
