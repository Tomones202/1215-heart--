...
import { GEMINI_MODEL_ANALYSIS, GEMINI_MODEL_IMAGE, GEMINI_MODEL_TTS, VOICE_OPTIONS, GEMINI_MODEL_ANALYSIS_FALLBACK, GEMINI_MODEL_IMAGE_FALLBACK } from "../constants";
...
export const analyzeProduct = async (
  ...
  **输出要求**：
  ...
  // Define common config to be reused
  ...
  try {
      ...
      // Fallback Logic: If 429 Resource Exhausted on Pro, try Flash
      if (isQuotaError(error)) {
          console.warn("Primary model (Pro) quota exhausted. Switching to Fallback Model (Flash)...");
          try {
             response = await withRetry(() => client.models.generateContent({
             ...
             throw fallbackError; // If fallback fails, propagate the error
          }
      } else {
          throw error; // If not a quota error (e.g., validation), throw immediately
      }
  }
...
export const generateImage = async (
  ...
  try {
    ...
    for (const part of response.candidates?.[0]?.content?.parts || []) {
      ...
      if (isQuotaError(error)) {
          console.warn("Image model (Pro) quota exhausted. Switching to Fallback Model (Flash Image)...");
          // Flash Image does NOT support `imageSize`, so we strip it from config
          const response = await withRetry<GenerateContentResponse>(() => client.models.generateContent({
            ...
            config: {
                imageConfig: {
                    aspectRatio: aspectRatio as any
                    // Note: imageSize is NOT supported by gemini-2.5-flash-image
                }
            }
          }));
